...
export class Engine {
    ...
    public expandAllRegions(document: vscode.TextDocument | null = null) {
        // Expand all folded regions in the current document
        if (!document) {
            var ate = vscode.window.activeTextEditor;
            if (!ate) { return; }
            document = ate.document;
        ...
        // Use VS Code's built-in command to unfold all
